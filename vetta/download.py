...
def download(
    ...
    *,
    ...
    """Download one or more VeTTA datasets from Zenodo into ``data_dir``.

    Thin public entry point over :func:`download_datasets`. ``dataset_names`` of
    ``None`` selects every supported dataset; ``dry_run`` only creates the target
    directories. For an offline end-to-end exercise of the pipeline (no network),
    use :func:`run_smoke_download` or the ``--smoke`` CLI flag.
    """
    return download_datasets(dataset_names=dataset_names, data_dir=data_dir, dry_run=dry_run)
